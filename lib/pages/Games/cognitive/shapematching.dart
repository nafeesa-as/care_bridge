...
class _ShapeMatchingGameState extends State<ShapeMatchingGame>
  ...
  void _checkAnswer(Map<String, dynamic> selectedShape) {
    if (selectedShape['shape'] == _targetShape['shape']) {
      ...
      if (_score >= 100) {
        // When score reaches 100, save and exit after a brief delay
        Future.delayed(const Duration(seconds: 1), () {
          _saveScore(); // This will save the score and exit
        });
      } else {
  ...
  Future<void> _saveScore() async {
    ...
    try {
      // Add to game_history collection with timestamp (this was missing)
      await _firestore
          .collection('users')
          .doc(user.uid)
      ...
      // Also update the scores collection for backward compatibility

...
class _HeightState extends State<Height> {
  ...
  Future<void> fetchGrowthChartData() async {
    try {
      ...
      if (snapshot.exists) {
        Map<String, dynamic> fullData = snapshot.data() as Map<String, dynamic>;
        Map<String, dynamic> boysData = fullData["boys"]["height"] ?? {};
        ...
        // Sort all lists by age (X-axis) to ensure smooth curves
        temp3rd.sort((a, b) => a.x.compareTo(b.x));
        temp10th.sort((a, b) => a.x.compareTo(b.x));
        temp25th.sort((a, b) => a.x.compareTo(b.x));
        temp50th.sort((a, b) => a.x.compareTo(b.x));
      ...
      child: Row(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              Text(
                "Age: ${latestAge.toStringAsFixed(1)} years",
                style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
                  color: Colors.white,
          ...
          Container(
            ...
            child: IconButton(
              icon: Icon(Icons.add, color: Colors.blue[900]),
              onPressed: () async {
                // Navigate to Add Measurement Page and refresh data when returning
                final result = await Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => EditHeightPage()),
                );

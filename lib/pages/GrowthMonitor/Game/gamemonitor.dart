...
extension DateTimeExtension on DateTime {
  // Improved startOfDay to ensure consistent timestamp handling
  DateTime startOfDay() {
    return DateTime(year, month, day, 0, 0, 0, 0, 0);
  }

  // Added endOfDay for more precise date range handling
  DateTime endOfDay() {
...
class _GameMonitorState extends State<GameMonitor> {
  ...
  void initState() {
    ...
    // Set selectedWeek to the Monday of the current week for consistency
    selectedWeek =
        selectedWeek
  ...
  Future<void> fetchWeeklyData() async {
    ...
    try {
      ...
      // Calculate start and end of selected week (Monday to Sunday)
      final mondayOfWeek =
          selectedWeek
              .subtract(Duration(days: selectedWeek.weekday - 1))
              .startOfDay();

      // Calculate the following Monday (exclusive end date)
  ...
  Future<void> fetchHealthData() async {
    ...
    try {
      final snapshot =
          await firestore.collection("scores").doc(loggedInUserId).get();
      ...
      if (snapshot.exists) {
        ...
        setState(() {
          ...
          bubblepopScore =
              (double.tryParse(data["colors"].toString()) ?? 0) / 100;
          tracingScore =
  ...
  // Updated week selector to show Monday's date consistently
  Widget buildWeekSelector() {
    ...
    // Calculate Sunday of the current week for display purposes
      ...
      children: [
        IconButton(
          ...
          onPressed: () {
            ...
            setState(() {
              selectedWeek = previousMonday; // Set to Monday directly
              fetchWeeklyData();
            });
          },
        ),
        ...
        IconButton(
          ...
          onPressed: () {
            ...
            if (nextMonday.isBefore(DateTime.now())) {
              setState(() {
                selectedWeek = nextMonday; // Set to Monday directly
                fetchWeeklyData();
              });
            }
          },
            ...
            isLoading
                ...
                ? Column(
                  ...
                  children: [
                    ...
                    // Add debug button in development to trigger manual refresh
                  ],
                )
                : Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    buildWeekSelector(), // Show week selector even when no data
                    const SizedBox(height: 20),
                    const Padding(

...
class _EditWeightPageState extends State<EditWeightPage> {
  ...
  Future<void> saveMeasurement() async {
    if (selectedDate == null || weightController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please select a date and enter weight."),
    ...
    try {
      ...
      Map<String, dynamic> existingWeightData = {};

      if (userData.containsKey('weight_data') &&
          userData['weight_data'] is Map) {
        userData['weight_data'].forEach((key, value) {
          existingWeightData[key.toString()] =
      ...
      existingWeightData[dateKey] = weightValue;

      await firestore.collection('users').doc(userId).set({
        'weight_data': existingWeightData,
